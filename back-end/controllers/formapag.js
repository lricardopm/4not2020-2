...
const Formapag = require('../models/Formapag')
...
controller.novo = async (req, res) => {
    ...
    try {
        await Formapag.create(req.body)
        // HTTP 201: Created
        res.status(201).end()
    }
    catch(erro) {
...
controller.listar = async (req, res) => {
    try {
        let dados = await Formapag.find() // Traz todos os Formapags cadastrados
...
controller.obterUm = async (req, res) => {
    try {
        ...
        let obj = await Formapag.findById(id)
...
controller.atualizar = async (req, res) => {
    try {
        ...
        let ret = await Formapag.findByIdAndUpdate(id, req.body)
...
controller.excluir = async (req, res) => {
    try {
        ...
        let ret = await Formapag.findByIdAndDelete(id)
